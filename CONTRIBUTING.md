...
- `ModuleStyleGraphRequestCache`：缓存一次 graph 请求中的 package context，避免同一次请求重复加载和扫描。
...
Vite 插件负责把 package CSS import 转成虚拟模块，并调用 `ModuleStyleGraph` 生成 CSS 内容。HMR 模块负责判断源码、配置、style 文件变化后哪些虚拟 CSS 模块需要失效。
...
flowchart TD
  ...
  Graph --> Cache["ModuleStyleGraphRequestCache"]
  Cache --> PackageContext["StylePackageContext"]
  Graph --> SharedPlan["style/plan.ts"]
...
- 新增 style 依赖类型时，同时检查 `dependencies.ts`、`workspaceStyleResolver.ts`、`moduleStyleImportCollector.ts` 和 `StyleStructure` 测试 helper。
