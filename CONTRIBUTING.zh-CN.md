...
- `moduleGraph/packageSource/`：抽象 dev graph packages 的来源。`singlePackage.ts` 使用 Vite root 作为当前 package root；`monorepo.ts` 扫描 workspace packages。
...
Publish 是建立在 auklet build 之上的辅助工作流。Publish 编排应留在 `src/publish/`，不要放进 `bin/entry.mjs`；CLI 只应该解析 auklet 自己消费的 flags、确保 pnpm 存在，并把 `PublishOptions` 交给 `PublishRunner`。

```text
src/publish/
├── cli.ts                 # publish/owner CLI flags 和校验
